...
/// <see cref="ExecuteAsync{TRequestType, TResponseType}(TRequestType, string, IBpServices, CancellationToken)"/>
...
public class RequestExecutionQueue<TRequestContext> : IRequestExecutionQueue<TRequestContext>
{
    ...
    private readonly AbstractBaseProtocolServer<TRequestContext> _baseProtocolServer;
    ...
    public RequestExecutionQueue(AbstractBaseProtocolServer<TRequestContext> baseProtocolServer, IBpLogger logger, IHandlerProvider handlerProvider)
    {
        _baseProtocolServer = baseProtocolServer;
        _logger = logger;
    ...
    private async Task ProcessQueueAsync()
    {
        ...
        catch (Exception ex)
        {
            ...
            {
                await _baseProtocolServer.ShutdownAsync("Error processing queue, shutting down").ConfigureAwait(false);
                await _baseProtocolServer.ExitAsync().ConfigureAwait(false);
            }
